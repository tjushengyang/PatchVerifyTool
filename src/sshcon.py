...
#         print( self.shell.recv(65535))   #收到打印返回    可以尝试不加decoding返回结果
        buff += recv
        output = buff
        return output  #返回结果输出        

if __name__ == '__main__':    #定义程序入口
    sshcon = SshCon(ip= "10.248.154.103", port = 22, username = "ppsicp", passwd = 'Psci@1234')  #定义初始值
    printout = sshcon.execute(cmd= 'pwd\n')   #输入指令
    print(printout)